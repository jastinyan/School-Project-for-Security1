...
function sendOTP($email, $otp) {
    ...
    try {
        ...
        $mail->SMTPAuth   = true;
        $mail->Username   = 'justinian.melecio@csucc.edu.ph';
        $mail->Password   = 'actkhrjutwdyevfa';
        $mail->SMTPSecure = PHPMailer::ENCRYPTION_STARTTLS;
        $mail->Port       = 587;

        $mail->setFrom('justinian.melecio@csucc.edu.ph', 'Meditation Activity Tracker');
...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST['verify'])) {
    ...
    if ($row = $result->fetch_assoc()) {
        ...
        elseif (password_verify($inputOtp, $row['otp'])) {
            ...
            header("Location: reset_password.php?id_no=$id_no");
            exit;
        } else {
